...
def main(cfg_path):
    ...
    # === Define the three experiment modes ===
    experiment_modes = [
        {"name": "spectral", "knn_filter": False, "majority_voting": False},
        {"name": "spatial_spectral", "knn_filter": True, "majority_voting": False},
        {"name": "majority_voting", "knn_filter": True, "majority_voting": True},
    ]

    for mode in experiment_modes:
        logger.info(f"\n🔹 Running setting: {mode['name'].upper()}")

        # --- Apply spatial/spectral KNN filtering if needed ---
        if mode["knn_filter"]:
            from utils.spatial_filtering import apply_knn_filter
            X_filtered = apply_knn_filter(X, cfg)
        else:
            X_filtered = X
        ...
        for fold_idx, train_set, val_set, test_set in make_kfold_splits(X_filtered, y, pids, cfg):
            ...
            for epoch in range(cfg.training.epochs):
                ...
                if (epoch + 1) % 10 == 0:
                    ...
                    logger.info(f"[{mode['name']} | Fold {fold_idx+1}] Epoch {epoch+1}: "
            ...
            if mode["majority_voting"]:
                ...
                metrics_test = majority_voting(metrics_test, y_test)

            results.append(metrics_test)
            logger.info(f"[{mode['name']} | Fold {fold_idx+1}] Test F1={metrics_test['f1_macro']:.3f}, OA={metrics_test['oa']:.3f}")
        ...
        out_dir = Path(cfg.experiment.output_dir) / f"{mode['name']}"
        ensure_dir(out_dir)
        df.to_csv(out_dir / "metrics_kfold.csv", index=True)
        logger.info(f"📊 Saved metrics_kfold.csv for setting '{mode['name']}' → {out_dir}")
