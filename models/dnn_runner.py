...
class DNN1D(nn.Module):
    # --- MODIFIED: Added dropout_rate ---
    def __init__(self, input_dim=128, num_classes=4, hidden_dims=(256, 256), dropout_rate=0.5):
        super().__init__()
        assert len(hidden_dims) == 2, "This paper-aligned DNN expects exactly two hidden layers."

        h1, h2 = hidden_dims

        # --- MODIFIED: Added Dropout ---
        # Layer 1: Linear -> ReLU -> Dropout
        layer1 = [
            nn.Linear(input_dim, h1),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout_rate) # Added regularization
        ]
        ...
        # Batch Norm *follows* the two hidden layers (as per your request)
        bn_layer = [
            nn.BatchNorm1d(h2)
        ]
...
class DNNRunner(BaseRunner):
    ...
    def __init__(self,
                 ...
                 epochs=300,
                 momentum=0.9,
                 # --- MODIFIED: Added regularization defaults ---
    ...
    def _train_one_model(self, net, X_train, y_train): # <-- NEW HELPER
        ...
        # Loss and optimizer
        criterion = nn.CrossEntropyLoss() # Unweighted loss (as per Run 5)
        ...
        for epoch in range(self.epochs):
            ...
            # --- CRITICAL BUG FIX ---
            # Scheduler must be stepped PER EPOCH, not per batch
            scheduler.step()
            # --- END BUG FIX ---
    ...
    def _evaluate_on_val(self, net, X_val, y_val): # <-- NEW HELPER
        ...
        net.eval() # <-- IMPORTANT: Disable dropout/BN updates for evaluation
        with torch.no_grad():
    ...
    def _predict_proba(self, X):
        ...
        self.net.eval() # <-- IMPORTANT: Disable dropout/BN updates for prediction
        with torch.no_grad():
            # Predict in batches
