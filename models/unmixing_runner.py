...
-----------
Implementa sia EBEAE (Lineare) che NEBEAE (Non-Lineare)
[cite_start]come descritto nel paper[cite: 934, 935].

Il modello viene selezionato all'avvio tramite il parametro 'name'.
"""
...
class UnmixingRunner(BaseRunner): # <-- MODIFICATO (Nome Classe)
    ...
    [cite_start]Se name="nebeae", esegue il modello bilineare[cite: 935].
    """
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        """
        Esegue il fit. Trova i parametri E (e B se NEBEAE) usando CCD,
        poi ottimizza 'y' (entropia) sul set di validazione.
        """
        if X_train.size == 0:
            print(f"[{self.name.upper()}] ⚠ Empty training set, skipping training.")
            return

        print(f"[{self.name.upper()}] 1. Fitting Model (E{', B' if self.is_nonlinear else ''}) using CCD...")
        E_fit, B_fit, class_map = self._fit_model_params(X_train, y_train)

        print(f"[{self.name.upper()}] 2. Optimizing 'y' (entropy weight) on validation set...")
        ...
        for y_current in self.y_search_space:
            ...
            print(f"[{self.name.upper()}]    y = {y_current:.4f}, Val F1 = {score:.4f}")
    ...
    def _update_parameters_als(self, X_target, A_features, class_map, class_p, X_train, y_train, is_bv_special_case=True):
        ...
        (Questa funzione è identica e riutilizzabile)
        """
    ...
    def _find_abundances(self, X_pixels, E, B, y_entropy_weight):
        ...
        Se B=None, esegue un unmixing lineare (EBEAE).
        Se B!=None, esegue un unmixing non-lineare (NEBEAE).
        """
